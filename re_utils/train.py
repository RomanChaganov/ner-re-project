...
def train_re(
    ...
    weights = torch.FloatTensor([1/2, 1/100]).to(torch.device(device))
...
def calc_f1_micro(
    ...
    for i, item in enumerate(test_re_dataset):
        ...
        true_ans = torch.sum((gt_relation_matrix_on_ner == 0) & (relation_matrix_pred == 0)).item()
        if nayden == 0:
            precision = 0
        else:
            precision += true_ans / nayden

        print(torch.sum(relation_matrix_pred == 0).item(), torch.sum(gt_relation_matrix_on_ner == 0).item(), true_ans)
